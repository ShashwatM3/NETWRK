    ...
    if (userData) {
      ...
      for (let att in userData) {
        if(att.includes("eventRSVPD_")) {
          console.log(att);
          if (allEvents && allEvents[userData[att].eventID]) {
            console.log(allEvents[userData[att].eventID].date);
            if(hasDatePassed(allEvents[userData[att].eventID].date)) {
              past[userData[att].eventID] = allEvents[userData[att].eventID];
            } else {
              curr[userData[att].eventID] = allEvents[userData[att].eventID];
            }
          }
        }
      }
      ...
      <React.Fragment>
        <div className='profile-main'>
          ...
          <div className='relative cardUser min-h-[40vh] m-6 mt-0 rounded-2xl p-6 pt-8 flex items-end'>
            ...
            <h3 className='absolute top-5 left-5 font-bold'>{userData[emailUser].netwrk_id}</h3>
